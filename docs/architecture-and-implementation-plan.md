...
```mermaid
flowchart LR
    UI[UI] -->|Create or update request| API[Node.js API]
...
1. The UI sends a request to the Node.js API.
2. The Node.js API validates and publishes an event to Kafka.
...
| Node.js API | Validate, create event envelopes, publish to Kafka | Read-only project queries |
...
There is no outbox worker in the active solution.

## Acceptance Criteria

The executable test at `tests/end-to-end/basic-flow.ps1` verifies:

- API database write count remains zero.
